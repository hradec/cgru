...
function g_ConfigReceived(i_obj)
{
	...
	if (i_obj.cgru_config)
	{
		...
		log += '<br>Server: ' + cgru_Environment.username + '@' + cgru_Environment.hostname + ':' +
...
function g_DigestInit(i_obj)
{
	...
	if (i_obj)
	{
		...
		if ((localStorage.digest == null) || (localStorage.realm == null) ||
			(localStorage.realm != i_obj.realm) || (localStorage.user_name == null) ||
...
function g_OpenMonitor(i_args)
{
	...
	if (i_args.evt)
	{
		if (i_args.evt.shiftKey)
			new_wnd = true;
		if (i_args.evt.ctrlKey)
			new_wnd = true;
		if (i_args.evt.altKey)
			new_wnd = true;
	}
...
function g_OpenWindowLoad(i_file, i_name)
{
	...
	var wnd =
		window.open('afanasy/browser/' + i_file, name, 'location=no,scrollbars=yes,resizable=yes,menubar=no');
	if (wnd == null)
	{
