...
function cgru_CmdExecFilter(i_cmd){return i_cmd;}
...
function cgru_RulesLink(i_path)
{
	...
	var abs_path = cgru_PM(i_path);
	if (abs_path && abs_path.length)
	{
		var abs_is_absolute = (abs_path.charAt(0) == '/') || (abs_path.charAt(0) == '\\');
		var orig_is_absolute = (i_path && ((i_path.charAt(0) == '/') || (i_path.charAt(0) == '\\')));
		if ((false == abs_is_absolute) && orig_is_absolute)
			abs_path = i_path;
	}
	...
	if (cgru_Config.rules_url && cgru_Config.rules_url.length)
	{
		var link = abs_path;
		for (var i = 0; i < cgru_Config.projects_root.length; i++)
		{
			var root = cgru_Config.projects_root[i];
		...
		link = link.replace(/\/\//g, '/');
		var base = cgru_Config.rules_url;
	...
	return cgru_ProjectServerLink(abs_path);
}

function cgru_ProjectServerLink(i_path)
{
	if ((i_path == null) || (i_path.length == 0))
		return null;

	var norm = i_path.replace(/\\/g, '/');
	var encoded = encodeURIComponent(norm).replace(/%2F/gi, '/');
...
function cgru_CmdExecCreate(i_args)
{
	...
	cgru_CmdExecProcess({"element": el, "open": i_open, "cmd": i_cmd, "cmds": i_cmds, "terminal": i_terminal});
...
function cgru_CmdExecOnDblClick(i_evt)
{
	...
	let payload = JSON.stringify({"headers": headers, "cmdexec": cmdexec});

	let sendToKeeper = function(i_use_https)
	{
		let xhr = new XMLHttpRequest;
		let url = 'http://localhost:' + cgru_Config.keeper_port + '/';
		if (i_use_https)
			url = 'https://localhost:' + cgru_Config.keeper_port_https + '/';

		xhr.open('POST', url, true);
		xhr._payload = payload;
		xhr.send(payload);
		xhr.onreadystatechange = cgru_CmdExecHandler;
		xhr.onerror = cgru_CmdExecHandler;
		xhr.m_el = el;
		xhr.m_use_https = i_use_https;
		xhr.m_tried_other = false;
		return xhr;
	};

	// Prefer HTTP unless the page itself is HTTPS (to avoid mixed content blocks and HTTPS certificate issues).
	let prefer_https = false;
	try { prefer_https = (window.location && window.location.protocol == 'https:'); } catch (err) {}

	if (prefer_https && cgru_Config.keeper_port_https)
		sendToKeeper(true);
	else if (cgru_Config.keeper_port)
		sendToKeeper(false);
	else if (cgru_Config.keeper_port_https)
		sendToKeeper(true);
}
...
function cgru_CmdExecHandler()
{
	...
	if (this.status == 0)
	{
		// Fallback between HTTPS and HTTP if one of them is unreachable (cert / mixed-content / CORS / service down).
		if (false == this.m_tried_other)
		{
			this.m_tried_other = true;

			let use_https = false;
			if (this.m_use_https)
				use_https = false;
			else if (cgru_Config && cgru_Config.keeper_port_https)
				use_https = true;

			let url = null;
			if (use_https && cgru_Config && cgru_Config.keeper_port_https)
				url = 'https://localhost:' + cgru_Config.keeper_port_https + '/';
			else if (false == use_https && cgru_Config && cgru_Config.keeper_port)
				url = 'http://localhost:' + cgru_Config.keeper_port + '/';

			if (url)
			{
			let xhr = new XMLHttpRequest;
				xhr.open('POST', url, true);
			xhr._payload = this._payload;
			xhr.send(this._payload || '');
			xhr.onreadystatechange = cgru_CmdExecHandler;
			xhr.onerror = cgru_CmdExecHandler;
			xhr.m_el = el;
				xhr.m_use_https = use_https;
				xhr.m_tried_other = true;
			return;
		}
		}
...
//	this.elParent.removeChild(this.elRoot); //< There was an error when one dialog handle function raises another one.
