...
function cgru_RulesLink(i_path)
{
	...
	// console.log('----->'+cgru_Config.rules_url);
	// console.log('----->'+link);



    // link = 'http://' + window.location.host.toString().split(':')[0]  + ':' + cgru_Config.rules_url.split(':')[2]  + link
	link = link.replace(/\/\//g, '/');
    link = 'http://' + cgru_Config.rules_url.replace('http://','')  + link
	console.log('+---->'+link);
...
function cgru_CmdExecOnDblClick(i_evt)
{
	...
	var cmds = [];
	if (el.m_cmd)
		cmds.push(cgru_CmdExecFilter(el.m_cmd));
	if (el.m_cmds)
